...
class ZephyrusLLM:
    ...
    def decompose_task(self, task_description):
        ...
        # Static part of the conversation context
        messages = [
          {
          "role": "system",
        ...
        output_text = self.tokenizer.batch_decode(generated_ids[:, input_length:], skip_special_tokens=True)[0]

        # Extracting tasks
        return self.extract_tasks(output_text)

    # Helper method to process chatbot output and extract tasks into a dictionary
    @staticmethod
    ...
    def generate_employee_review(self, data, employee_name):
        set_seed(0)
        df = data
        employee_data = self.get_employee_data(df, employee_name)
        context = self.prepare_context(employee_data)

        # Few-shot prompting with the context
        messages = [
        ...
        generated_ids = self.model.generate(model_inputs, do_sample=True, max_new_tokens=200)
        review = self.tokenizer.batch_decode(generated_ids[:, input_length:], skip_special_tokens=True)[0]
        return review
